...
impl CodexConfig {
    ...
    pub(crate) fn effective_agent_id(&self) -> &str {
...
/// ```
pub(crate) fn discover_codex() -> Result<CodexConfig> {
...
#[allow(dead_code)]
pub(crate) fn format_for_codex(msg: &Message) -> String {
...
#[derive(Debug, Clone)]
pub(crate) struct NormalizedFinding {
    /// Original message ID.
...
/// ```
/// # use agent_bus::models::Message;
/// # use agent_bus::codex_bridge::normalize_findings;
/// // normalize_findings(&messages) returns only finding-type messages.
/// ```
pub(crate) fn normalize_findings(messages: &[Message]) -> Vec<NormalizedFinding> {
    messages
        .iter()
        .filter(|m| {
...
pub(crate) struct CodexSyncResult {
    ...
    pub(crate) messages_read: usize,
    /// Number of finding-type messages normalized.
    pub(crate) findings_normalized: usize,
    /// Agent ID of the discovered Codex instance.
    pub(crate) codex_agent_id: String,
...
"#;
    ...
    fn format_for_codex_includes_all_sections() {
        let msg = Message {
            ...
            thread_id: None,
            tags: vec!["repo:my-project".to_owned()],
