...
pub fn bus_post_message_with_notifications(
    ...
    let stream_id: String = redis::cmd("XADD")
        ...
        .arg(&fields)
        .query(conn)
        .map_err(|e| crate::error::AgentBusError::Internal(format!("XADD failed: {e}")))?;
    ...
    if should_publish_message_event(&msg, has_sse_subscribers) {
        ...
        let _: () = conn
            ...
            .map_err(|e| crate::error::AgentBusError::Internal(format!("PUBLISH failed: {e}")))?;
    }
...
pub fn bus_read_page_from_redis(
    ...
    let raw: Vec<redis::Value> = redis::cmd("XRANGE")
        ...
        .arg(limit)
        .query(&mut conn)
        .map_err(|e| crate::error::AgentBusError::Internal(format!("XRANGE failed: {e}")))?;
...
pub fn bus_list_presence(
    ...
    loop {
        let (next_cursor, batch): (u64, Vec<String>) = redis::cmd("SCAN")
            ...
            .arg(100)
            .query(conn)
            .map_err(|e| crate::error::AgentBusError::Internal(format!("SCAN failed: {e}")))?;
        keys.extend(batch);
        cursor = next_cursor;
        if cursor == 0 {
            break;
...
pub fn pull_task(settings: &Settings, agent: &str) -> Result<Option<String>> {
    ...
    let task: Option<String> = redis::cmd("LPOP")
        .arg(&key)
        .query(&mut conn)
        .map_err(|e| crate::error::AgentBusError::Internal(format!("LPOP task failed: {e}")))?;
    Ok(task)
}
...
pub fn peek_tasks(settings: &Settings, agent: &str, limit: usize) -> Result<Vec<String>> {
    ...
    let tasks: Vec<String> = redis::cmd("LRANGE")
        ...
        .arg(end)
        .query(&mut conn)
        .map_err(|e| crate::error::AgentBusError::Internal(format!("LRANGE tasks failed: {e}")))?;
    Ok(tasks)
}
    ...
    let overdue = all
        ...
        .filter(|d| {
            chrono::DateTime::parse_from_rfc3339(&d.deadline_at.replace('Z', "+00:00"))
                .is_ok_and(|ts| ts.with_timezone(&chrono::Utc) < now)
        })
        .collect();
    Ok(overdue)
}
