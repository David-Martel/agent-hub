...
pub fn output<T: Serialize + ?Sized>(data: &T, encoding: &Encoding) {
    match encoding {
        Encoding::Json => {
...
pub fn output_messages(msgs: &[Message], encoding: &Encoding) {
...
pub fn output_presence(p: &Presence, encoding: &Encoding) {
    match encoding {
        Encoding::Human => {
            println!(
                "[{}] presence {}={} session={}",
...
/// ```
#[must_use]
pub fn format_health_toon(h: &Health) -> String {
...
/// ```
#[must_use]
pub fn format_message_toon(msg: &Message) -> String {
...
/// ```
#[must_use]
pub fn format_presence_toon(p: &Presence) -> String {
...
pub fn minimize_value(value: &serde_json::Value) -> serde_json::Value {
    match value {
...
///
/// # Errors
/// Returns an error if `data` is not valid `MessagePack`.
// allow: pub API for future HTTP/MCP binary wire protocol; currently used in tests only.
#[allow(dead_code)]
pub fn decode_msgpack(data: &[u8]) -> Result<serde_json::Value, rmp_serde::decode::Error> {
