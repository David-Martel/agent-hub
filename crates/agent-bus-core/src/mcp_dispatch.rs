...
impl<'a> McpToolDispatch<'a> {
    ...
    pub fn dispatch_tool(
        ...
        match name {
            ...
            "post_message" => {
                let sender = get_str(args, "sender").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("sender is required".to_string())
                })?;
                let recipient = get_str(args, "recipient").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("recipient is required".to_string())
                })?;
                let topic = get_str(args, "topic").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("topic is required".to_string())
                })?;
                let body = get_str(args, "body").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("body is required".to_string())
                })?;
            ...
            "ack_message" => {
                let agent = get_str(args, "agent").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("agent is required".to_string())
                })?;
                let message_id = get_str(args, "message_id").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("message_id is required".to_string())
                })?;
            ...
            "create_channel" => {
                let channel_type = get_str(args, "channel_type").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("channel_type is required".to_string())
                })?;
                let created_by = get_str(args, "created_by").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("created_by is required".to_string())
                })?;
                    ...
                    "group" => {
                        let name = get_str(args, "name").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "name is required for group channels".to_string(),
                            )
                        })?;
            ...
            "post_to_channel" => {
                ...
                match channel_type {
                    "direct" => {
                        let recipient = get_str(args, "recipient").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "recipient is required for direct channels".to_string(),
                            )
                        })?;
                    ...
                    "group" => {
                        let group_name = get_str(args, "recipient").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "recipient (group name) is required for group channels".to_string(),
                            )
                        })?;
            ...
            "read_channel" => {
                ...
                match channel_type {
                    "direct" => {
                        let agent_a = get_str(args, "agent_a").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "agent_a is required for direct channels".to_string(),
                            )
                        })?;
                        let agent_b = get_str(args, "agent_b").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "agent_b is required for direct channels".to_string(),
                            )
                        })?;
                    ...
                    "group" => {
                        let group_name = get_str(args, "group_name").ok_or_else(|| {
                            crate::error::AgentBusError::Internal(
                                "group_name is required for group channels".to_string(),
                            )
                        })?;
            ...
            "claim_resource" => {
                let resource = get_str(args, "resource").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("resource is required".to_string())
                })?;
                let agent = get_str(args, "agent").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("agent is required".to_string())
                })?;
            ...
            "resolve_claim" => {
                ...
                let winner = get_str(args, "winner").ok_or_else(|| {
                    crate::error::AgentBusError::Internal("winner is required".to_string())
                })?;
